...
#       [{information about a residue that is whithin the search radius}, {}, {}, {}]
    ...
    lResidues = []
    for i in range(0,len(unRes)):
        if len(unRes[i][0].atoms.select_atoms(searchAtoms)) > 0: #ignore the obvious fact that the search atoms are within x angstroms of themselves
            continue
        dRes = {}
